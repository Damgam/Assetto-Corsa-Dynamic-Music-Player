...
NowPlayingWidgetDir = ConfigFile:get("settings", "nowplayingwidgetdir", "Default")
NowPlayingWidgetSize = ConfigFile:get("settings", "nowplayingscale", 1)
EnableOBSExport = ConfigFile:get("settings", "enableobsexport", false)
...
WidgetsDir = '/NowPlaying Widgets'
WidgetsList = table.map(io.scanDir( __dirname .. WidgetsDir, '*'), function (x) return { string.sub(x, 1, #x - 4), WidgetsDir .. '/' .. x } end)
table.sort(WidgetsList, function(a,b) return string.upper(a[1]) < string.upper(b[1]) end)

function NowPlayingWidgetTab()
    checkbox = ui.checkbox("Enable OBS Export (Can cause stutters on track switch)", EnableOBSExport)
    if ui.itemHovered() then
        ui.setTooltip('Exports track title and cover art to OBS Stuff folder, so you can use it for displaying outside of the game, not necesarily in OBS')
    end
    if checkbox then
        EnableOBSExport = not EnableOBSExport
        ConfigFile:setAndSave("settings", "enableobsexport", EnableOBSExport)
    end
    ui.separator()
    ui.separator()
    ...
    ui.text('Widget Selection:')
    ui.separator()
    ui.separator()
    for i = 1, #WidgetsList do
        if io.fileExists(ac.getFolder(ac.FolderID.ACApps) .. "/lua/DynamicMusicPlayer" .. WidgetsList[i][2] .. "/Widget.lua") then
            ui.button(string.sub(WidgetsList[i][2], 21))
            if ui.itemHovered() then
                ui.setTooltip('Switch (Restarts the music player)')
            end
            if ui.itemClicked(ui.MouseButton.Left, false) then
                NowPlayingWidgetDir = string.sub(WidgetsList[i][2], 21)
                ConfigFile:setAndSave("settings", "nowplayingwidgetdir", string.sub(WidgetsList[i][2], 21))
                ac.restartApp()
                ac.setClipboadText(string.sub(WidgetsList[i][2], 21))
            end
            if string.sub(WidgetsList[i][2], 21) == NowPlayingWidgetDir then
                ui.sameLine()
                ui.dwriteText("⬅")
            end
        else
            ui.text(string.sub(WidgetsList[i][2], 21) .. " -")
            ui.sameLine()
            ui.text("MISSING Widget.lua")
        end
    end
    ui.separator()
    ui.separator()
...
if io.fileExists(ac.getFolder(ac.FolderID.ACApps) .. "/lua/DynamicMusicPlayer/NowPlaying Widgets/" .. NowPlayingWidgetDir .. "/Widget.lua") then
    require('NowPlaying Widgets/' .. NowPlayingWidgetDir .. '/Widget')
else
    require('NowPlaying Widgets/Default/Widget')
end
