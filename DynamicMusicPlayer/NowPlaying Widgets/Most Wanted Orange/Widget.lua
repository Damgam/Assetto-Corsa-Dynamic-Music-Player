EnableCoverArt = ConfigFile:get("nowplayingMostWantedOrange", "CoverArt", true)
EnableWidgetFadeout = ConfigFile:get("nowplayingMostWantedOrange", "FadeoutEnabled", true)
SpinCoverArt = ConfigFile:get("nowplayingMostWantedOrange", "SpinCoverArt", false)
WidgetFadeoutTime = ConfigFile:get("nowplayingMostWantedOrange", "DisplayTime", 15)
...
function NowPlayingOptions()
    ...
    if checkbox then
        ...
        ConfigFile:setAndSave("nowplayingMostWantedOrange", "CoverArt", EnableCoverArt)
    end

    checkbox = ui.checkbox("Spin Cover Art", SpinCoverArt)
    if checkbox then
        SpinCoverArt = not SpinCoverArt
        ConfigFile:setAndSave("nowplayingMostWantedOrange", "SpinCoverArt", SpinCoverArt)
    end

    checkbox = ui.checkbox("Enable Widget Fadeout", EnableWidgetFadeout)
    if checkbox then
        EnableWidgetFadeout = not EnableWidgetFadeout
        ConfigFile:setAndSave("nowplayingMostWantedOrange", "FadeoutEnabled", EnableWidgetFadeout)
    end

    if EnableWidgetFadeout then
        ui.text('Show the widget for first ' .. math.ceil(WidgetFadeoutTime) .. ' seconds of new track')
        local sliderValue7 = ConfigFile:get("nowplayingMostWantedOrange", "DisplayTime", 15)
        ...
        if WidgetFadeoutTime ~= sliderValue7 then
            ...
            ConfigFile:setAndSave("nowplayingMostWantedOrange", "DisplayTime", sliderValue7)
...
local function renderNowPlayingMainCanvas()
    ...
    if SpinnerFadeIn > 0 then
        ...
        ui.drawImage(ac.getFolder(ac.FolderID.ACApps) .. "/lua/DynamicMusicPlayer/NowPlaying Widgets/Most Wanted Orange/TRAX.png", vec2(275+(StringLength*StringProgress), 112), vec2(430+(StringLength*StringProgress), 312), rgbm(1,1,1,StringProgress))
        ui.drawImage(ac.getFolder(ac.FolderID.ACApps) .. "/lua/DynamicMusicPlayer/NowPlaying Widgets/Most Wanted Orange/BAR.png", vec2(196, 112), vec2(352+(StringLength*StringProgress), 312), rgbm(1,1,1,StringFadeIn))
        ...
        ui.drawImage(ac.getFolder(ac.FolderID.ACApps) .. "/lua/DynamicMusicPlayer/NowPlaying Widgets/Most Wanted Orange/Art Ring.png", vec2(100, 100), vec2(326, 326), rgbm(1,1,1,ArtRingFadeIn))
        if SpinCoverArt then
            ui.endRotation(SpinnerRotation)
        end
        ui.beginRotation()
        ui.drawImage(ac.getFolder(ac.FolderID.ACApps) .. "/lua/DynamicMusicPlayer/NowPlaying Widgets/Most Wanted Orange/Spinner.png", vec2(100, 100), vec2(326, 326), rgbm(1,1,1,SpinnerFadeIn))
        ui.endRotation(SpinnerRotation)
